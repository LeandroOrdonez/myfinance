...
const ComparisonSavingsChart: React.FC<ComparisonSavingsChartProps> = ({ data }) => {
  ...
  const hasRealData = data?.real_savings_series && Object.keys(data.real_savings_series).length > 0;
  ...
  const activeSeries = showReal && hasRealData ? data.real_savings_series! : data.savings_series;
